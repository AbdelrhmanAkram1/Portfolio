...
// About photo — mouse-following 3D tilt with a shadow that shifts with it
// (falls back to the static CSS tilt otherwise)
(function aboutPhotoTilt(){
  ...
  const maxTilt = 14;
  const defaultTransform = "rotateY(-10deg) rotateX(5deg)";
...
// Dynamic hero line chart — gently animates the data points, like a live feed
(function animateHeroChart(){
  const polyline = document.getElementById("heroPolyline");
  if (!polyline) return;

  const basePoints = [
    [10, 220], [80, 180], [150, 230], [220, 120], [290, 150], [360, 60], [410, 90]
  ];
  ...
  function frame(t){
    ...
    const points = basePoints.map(([x, y], i) => {
      const offset = Math.sin(time * 0.6 + i * 0.9) * 9;
      return [x, y + offset];
    });
