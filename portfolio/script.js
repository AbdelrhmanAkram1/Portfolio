...
// Hero photo — entrance animation is handled via CSS (heroPhotoIn keyframes),
// so only the mouse-tilt behavior needs JS here.

// Photo tilt — mouse-following 3D tilt with a shadow that shifts with it
// (falls back to the static CSS tilt otherwise).
function initPhotoTilt(wrapSelector, defaultTransform){
  ...
  const maxTilt = 14;
  const defaultShadow = "-18px 22px 46px rgba(0,0,0,.35), 0 0 0 1px var(--border)";
...
// Section backgrounds — a subtle animated data network (nodes drifting, connecting
// lines forming and breaking), evoking live data analysis without being loud.
// Applied to every section's canvas, each running independently.
function initDataNetwork(canvas){
  ...
  function initNodes(){
    const density = width < 700 ? 40 : 70;
    const count = Math.round((width * height) / (density * 900));
    nodes = Array.from({ length: Math.max(10, Math.min(count, 40)) }, () => ({
      x: Math.random() * width,
  ...
  function step(){
    ...
    const primary = getColor("--primary") || "95,168,211";
    const rgb = primary.startsWith("#") ? hexToRgb(primary) : primary;
  ...
  if (!reduceMotion){
    ...
    // Reduced motion: redraw a single static frame on resize/theme change, no loop.
    window.addEventListener("resize", () => { resize(); initNodes(); step(); });
  }

  // Keep colors correct when the person toggles dark/light mode
  const themeBtn = document.getElementById("themeToggle");
  if (themeBtn) themeBtn.addEventListener("click", () => step());
}

document.querySelectorAll(".data-canvas").forEach(initDataNetwork);
